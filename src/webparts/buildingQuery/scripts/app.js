var $app = {
  ...
  defaultProgramErrorHandler: function (jqXHR, textStatus) {
    ...
    alert(
      "There was a problem executing your request. Please try again. If the problem persists please relaunch the app or refresh the page."
    );
  },
  ...
  appendSPQueryToUrl: function (/*string*/ url) {
    ...
    url += url.indexOf("?") > -1 ? "&" : "?";
    url +=
      "SPHostUrl=" + encodeURIComponent($app.getUrlParamByName("SPHostUrl"));
    url +=
      "&SPAppWebUrl=" +
      encodeURIComponent($app.getUrlParamByName("SPAppWebUrl"));
    url +=
      "&SPLanguage=" + encodeURIComponent($app.getUrlParamByName("SPLanguage"));
    url +=
      "&SPClientTag=" +
      encodeURIComponent($app.getUrlParamByName("SPClientTag"));
    url +=
      "&SPProductNumber=" +
      encodeURIComponent($app.getUrlParamByName("SPProductNumber"));
  ...
  ensureContextQueryString: function () {
    ...
    if (queryString.indexOf(SPHasRedirectedToSharePointParam) >= 0) {
      window.location.search = queryString.replace(
        SPHasRedirectedToSharePointParam,
        ""
      );
    }
  ...
  onStart: function (/*function()*/ onStartFunc) {
    if ($app.onStartPromise == null) {
      $app.onStartPromise = $.when(
        $.Deferred(function (d) {
          $(d.resolve);
        }).then(
          $.Deferred(function (d) {
            $app.ensureContextQueryString();
            d.resolve();
          })
        )
      ).promise();
