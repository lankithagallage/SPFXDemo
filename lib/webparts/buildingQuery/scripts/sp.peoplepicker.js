  ...
  $.fn.spPeoplePicker = function (options) {
    var methods = {
      init: function () {
        ...
        container.append(
          '<div class="dropdown sp-peoplepicker-wrapper"><input type="hidden" /><div class="sp-peoplepicker-selectedValues"></div><input type="text" class="form-control sp-peoplepicker-editInput" data-toggle="dropdown" /><ul class="dropdown-menu sp-peoplepicker-dropdown" role="menu" sp-peoplepicker-lastticks="0"></ul></div>'
        );

        // delegate events to the container to maintain scope
        container.on(
          "keypress",
          "input.sp-peoplepicker-editInput",
          methods.searchUser
        );
        ...
        container.on(
          "click",
          "ul.sp-peoplepicker-dropdown > li",
          methods.selectUser
        );
        ...
        container.on(
          "click",
          "span.sp-peoplepicker-removeChosen",
          methods.removeChosenUser
        );
      ...
      selectUser: function (e) {
        ...
        if (arr.length >= settings.maxSelectedUsers) {
          alert(
            "This people picker is configured to allow " +
              settings.maxSelectedUsers +
              " users, please remove a selected user."
          );
          return;
        }
      ...
      setSelectedUsers: function (
        ...
        for (var i = 0; i < arr.length; i++) {
          var chosen = arr[i];
          controlContext.selectUsersDiv.append(
            '<span class="sp-peoplepicker-chosen" sp-login="' +
              chosen.login +
              '">' +
              chosen.displayName +
              '<span class="sp-peoplepicker-removeChosen"><span class="glyphicon glyphicon-remove"></span></span></span>'
          );
        }
      },
      ...
      searchUser: function (e) {
        ...
        var currentValue =
          controlContext.userEditInput.val() + String.fromCharCode(e.which);

        if (
          currentValue === null ||
          currentValue.length < settings.minSearchTriggerLength
        ) {
        ...
        controlContext.dropdown.empty();
        controlContext.dropdown.append(
          '<li role="presentation">Searching...</li>'
        );

        $app
          .withSPContext(function (spContext) {
            var queryTerm =
              "" + settings.searchPrefix + currentValue + settings.searchSuffix;

            var query =
              new SP.UI.ApplicationPages.ClientPeoplePickerQueryParameters();
            query.set_allowMultipleEntities(false);
            query.set_maximumEntitySuggestions(
              settings.maximumEntitySuggestions
            );
          ...
          .fail(function () {
            controlContext.dropdown.empty();
            alert(
              "There was a problem connecting to SharePoint. Please refresh the page to try again."
            );
          });
      },
      ...
      searchSuccess: function () {
        ...
        var lastTicks = parseFloat(
          controlContext.dropdown.attr("sp-peoplepicker-lastticks")
        );
        ...
        controlContext.dropdown.attr(
          "sp-peoplepicker-lastticks",
          this.queryTicks
        );
        ...
        // parse out results
        var results = spContext.parseObjectFromJsonString(
          this.result.get_value()
        );
        ...
        if (results.length < 1) {
          ...
          for (var i = 0; i < displayCount; i++) {
            ...
            foundItems.push(
              '<li role="presentation" class="sp-peoplepicker-foundItem" sp-login="' +
                loginName +
                '" sp-title="' +
                title +
                '" sp-email="' +
          ...
          var ofStr =
            results.length < settings.maximumEntitySuggestions
              ? "" + results.length
              : settings.maximumEntitySuggestions + "+";

          controlContext.dropdown.append(
            '<li role="presentation" class="divider"></li><li role="presentation">Showing ' +
              displayCount +
              " items of " +
              ofStr +
              "</li>"
      ...
      searchFail: function () {
        alert(
          "There was a problem executing your search. Please try again, if the problem persists please refresh the page."
        );
      },

      getControlContext: function (/*jQuery*/ elementInControlContainer) {
        var container = elementInControlContainer.hasClass(
          "sp-peoplepicker-wrapper"
        )
          ? elementInControlContainer
          : elementInControlContainer.closest("div.sp-peoplepicker-wrapper");
